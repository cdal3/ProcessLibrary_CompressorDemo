...
public class raP_5_20_PINTLK_NL_HomeLoadBanks : BaseNetLogic
{
    ...
    // Cancellation flag — set in Stop() to abort LoadBanks() early if panel is destroyed
    ...
    public override void Start()
    {
        ...
        // Use LongRunningTask so the UI container renders first, then banks load progressively
    ...
    private void LoadBanks()
    {
        ...
        const string LOGID = "Interlock Home: Load Banks";
        const string LIST_WIDGET_NAME = "raP_5_20_PINTLK_HomeList";
        const string BANK_WIDGET_NAME = "raP_5_20_PINTLK_HomeListBank";
        ...
        if (valBankMap == 0)
        {
            // No banks — just a single interlock list widget
            try
            {
        ...
        {
            // Progressively load each bank widget one at a time so UI updates incrementally
            try
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    // Check cancellation before creating each widget
                    if (stopping) return;
