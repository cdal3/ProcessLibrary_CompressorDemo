...
public class raP_5_20_raP_Opr_Seq_MultiStepCfg_BoolOutput_NL : BaseNetLogic
{
    ...
    public void Init(NodeId panelLoaderNodeId)
    {
        ...
        Log.Info("MultiStepCfg_BoolOutput_NL", $"Init token={myToken}");
        ...
        _debounceTask = new DelayedTask(() =>
        {
            ...
            try
            {
                ...
                if (panelLoader == null)
                {
                    Log.Warning("MultiStepCfg_BoolOutput_NL", "PanelLoader not found");
                    return;
                }
            ...
            {
                Log.Error("MultiStepCfg_BoolOutput_NL", $"Failed to Init: {ex}");
            }
    ...
    private void WaitForPanelAndInit(PanelLoader panelLoader, long token, int retryCount)
    {
        ...
        if (retryCount >= 10)
        {
            Log.Warning("MultiStepCfg_BoolOutput_NL", "Panel load timeout");
            return;
        }
    ...
    private void DoInit(PanelLoader panelLoader, long token)
    {
        ...
        try
        {
            ...
            // Container for step rows (keep old UI until confirmed)
                ...
                vSetEditStepRel == null || vCurrState == null)
            {
                ...
                Log.Warning("MultiStepCfg_BoolOutput_NL",
        ...
        {
            Log.Error("MultiStepCfg_BoolOutput_NL", $"DoInit failed: {ex}");
            if (loadingVisibleVar != null) loadingVisibleVar.Value = false;
        }
    }

    // Strict PLC confirmation: ALWAYS RemoteRead to confirm
        ...
        int retry = 0)
    {
        ...
        var task = new DelayedTask(() =>
        {
            ...
            {
                Log.Warning("MultiStepCfg_BoolOutput_NL", $"Remote sync check failed: {ex.Message}");
            }
    ...
    private void CreateInstance(IUANode parent, int startStep, int stepsOnPage)
    {
        ...
        {
            var instance =
                InformationModel.MakeObject<raP_5_20_raP_Opr_Seq_MultiStepCfg_BoolOutput>(
                    $"MultiStepCfg_BoolOutput{j}");
